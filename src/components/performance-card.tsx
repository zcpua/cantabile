...
export function PerformanceCard({ performance }: { performance: Performance }) {
  return (
    <Card className="h-full">
      <p className="eyebrow">{formatPerformanceDate(performance.startsAt)}</p>
      <h3 className="mt-3 font-serif text-2xl font-semibold leading-tight text-ink">{performance.title}</h3>
      <div className="mt-4 space-y-2 text-sm text-muted">
        <p><span className="font-medium text-ink">城市</span>：{performance.city}</p>
        <p><span className="font-medium text-ink">场馆</span>：{performance.venue}</p>
        <p><span className="font-medium text-ink">演出者</span>：{performance.artists.join("、")}</p>
      </div>
